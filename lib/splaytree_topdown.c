...
/// enter the discussion when considering allocator schemes. This is a topdown
/// splay tree implementation based on the work of Daniel Sleator (Carnegie
/// Mellon University). There are significant modifications to consider in the
/// context of a heap allocator, however. Firstly, splay trees do not support
/// duplicates yet we must do so for a heap allocator. Secondly, we must track
/// parents even though topdown splay trees do not require us to do so. We will
/// save space by combining the duplicate logic with the parent tracking logic.
/// This is required because of coalescing and the possibility of a child node
/// changing as a result of a coalesce requiring an update for the parent. There
/// are many optimizations focused on speeding up coalescing and handling
/// duplicates. See the detailed writeup for more information or explore the
/// code. Citations:
/// ------------------------------------------
...
/// A node in this splay tree does not track its parent. We also have the
/// additional optimization of tracking duplicates in a linked list of
/// duplicate_node members. We can access that list via the list_start. If the
/// list is empty it will point to our placeholder node that acts as nil.
struct Node {
    Header header;
    struct Node *links[2];
...
static struct Free_nodes {
    ...
    // between tree and list. The nil also searves as a dummy for the furthest
    // right end of our heap.
    struct Node *nil;
...
static struct Node *get_node(void const *client_space);
static void give_parent_subtree(struct Node *parent, enum Tree_link dir,
                                struct Node *subtree);
...
static bool is_tree_mem_valid(const struct Node *root, void const *nil_and_tail,
...
static bool is_duplicate_storing_parent(const struct Node *parent,
                                        ...
                                        void const *nil_and_tail);
static void print_tree(const struct Node *root, void const *nil_and_tail,
...
/// splay tree. For more
///        information on this interpretation see any Stanford 166 lecture on
///        splay trees.
void
...
free_coalesced_node(void *to_coalesce) {
    ...
    if (lft_tree_node != free_nodes.nil
        ...
        // Coalescing the first node in linked list. Dummy head, aka
        // lft_tree_node, is to the left.
...
static struct Node *
splay_bestfit(struct Node *root, size_t key) {
    ...
    for (;;) {
        ...
        // Because topdown rotations may move the best fit to the left or right
        // subtrees we are building we need to check if the best fit is one of
        // the children before potentially rotating/linking them away.
        best_fit
            = root_size < best_fit && root_size >= key ? root_size : best_fit;
        size_t left_child_size = get_size(root->links[L]->header);
        best_fit = left_child_size < best_fit && left_child_size >= key
                       ? left_child_size
                       : best_fit;
        size_t right_child_size = get_size(root->links[R]->header);
        best_fit = right_child_size < best_fit && right_child_size >= key
                       ? right_child_size
                       : best_fit;
        enum Tree_link link_to_descend = root_size < key;
        if (key == root_size
            || root->links[link_to_descend] == free_nodes.nil) {
            break;
        }
        ...
        if (key != child_size
            && link_to_descend == link_to_descend_from_child) {
            finger = root->links[link_to_descend];
            give_parent_subtree(root, link_to_descend,
        ...
        give_parent_subtree(left_right_subtrees[!link_to_descend],
    ...
    // We need bestfit which is not what splaytrees are made for. We can't
    // rewind all the fixups or cherry pick the best fit; we need splay
    // operations to fixup the tree for our selection. So, we will run another
    // search for that specific bestfit value. Total, that's 2 traversals in
    // worst case.
...
static struct Node *
splay(struct Node *root, size_t key) {
    // Pointers in an array and we can use the symmetric enum and flip it to
    // choose the Left or Right subtree. Another benefit of our nil node: use it
    // as our helper tree because we don't need its Left Right fields.
    free_nodes.nil->links[L] = free_nodes.nil;
...
insert_node(struct Node *current) {
    ...
    if (current_key == found_size) {
        ...
        add_duplicate(free_nodes.root, (struct Duplicate_node *)current,
                      free_nodes.nil);
        return;
    }
...
/// @warning this function has proven to be VERY important. The nil node often
/// has garbage values associated with real nodes in our tree and if we access
/// them by mistake it's bad!
static inline void
give_parent_subtree(struct Node *parent, enum Tree_link dir,
                    struct Node *subtree) {
    parent->links[dir] = subtree;
    if (subtree != free_nodes.nil
        && subtree->list_start != free_nodes.list_tail) {
...
static inline struct Node *
get_node(void const *client_space) {
...
static size_t
extract_tree_mem(const struct Node *root, void const *nil_and_tail) {
...
static bool
is_tree_mem_valid(const struct Node *root, void const *nil_and_tail,
                  size_t total_free_mem) {
...
static void
print_node(const struct Node *root, void const *nil_and_tail,
           enum Print_style style) {
...
print_inner_tree(const struct Node *root, size_t parent_size,
                 char const *prefix, char const *prefix_branch_color,
    ...
    char const *left_edge_color = get_edge_color(root->links[L], subtree_size);
...
static void
print_tree(const struct Node *root, void const *nil_and_tail,
           enum Print_style style) {
    if (root == nil_and_tail) {
        return;
    }
