...
///  relying on the properties of a red-black tree to remain balanced.
///  Citations:
///  -------------------
...
///    lives on the heap.
///  - Every path from root to tree.black_nil, root not included, has same
...
// NOT(!) operator will flip this enum to the opposite field. !L == R and !R ==
// L;
enum Tree_link {
    // (L == LEFT), (R == RIGHT)
    L = 0,
...
winit(void *heap_start, size_t heap_size) {
    ...
    tree.black_nil->parent = tree.black_nil->links[L] = tree.black_nil->links[R]
        = NULL;
...
delete_rb_node(struct Rb_node *remove) {
    ...
    if (remove->links[L] == tree.black_nil
        || remove->links[R] == tree.black_nil) {
...
rb_transplant(const struct Rb_node *remove, struct Rb_node *replacement) {
    if (remove->parent == tree.black_nil) {
        ...
        // Store the link from parent to child. True == 1 == R, otherwise False
        // == 0 == L
...
fix_rb_insert(struct Rb_node *current) {
    while (get_color(current->parent->header) == RED) {
        // Store the link from ancestor to parent. True == 1 == R, otherwise
        // False == 0 == L
        enum Tree_link symmetric_case
...
extract_tree_mem(const struct Rb_node *root, const struct Rb_node *black_nil) {
    ...
    return get_size(root->header) + HEADERSIZE
           + extract_tree_mem(root->links[R], black_nil)
...
print_node(const struct Rb_node *root, const struct Rb_node *black_nil,
    ...
    if (style == VERBOSE) {
        ...
        printf("%s(bh: %d)%s", COLOR_BLK, get_black_height(root, black_nil),
               COLOR_NIL);
    }
    printf("\n");
}
