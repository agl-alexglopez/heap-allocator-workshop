...
/// -------------------------------
/// This file contains tests specific to the list_addressorder allocator. Add
/// more tests if needed here to ensure a correct implementation of the
/// allocator. Notice that we use our segment of memory with consistent addresses
/// to help form better tests for the functionality of the allocator. For a real
/// allocator, we would alter our approach by not relying on the same segment
/// with identical addresses across successive runs.
/// If a test fails we have inserted breakpoint() that will only activate while
/// debugging in gdb. This makes it easy to tell exactly when we fail and where
/// in the heap we are when a test raises a problem.
...
/// @brief check_init     checks the internal representation of our heap, especially the head and tail
...
{
    // We also need to make sure the leftmost header always says there is no space
    // to the left.
    ...
    /* There is one very rare edgecase that may affect the next field of the
     * free_list_tail. This is acceptable because we never use that field and do
     * not need it to remain NULL.
     */
    if ( head->prev != NULL ) {
        return false;
    }
...
/// @brief is_free_list_valid  loops through only the doubly linked list to make sure it matches the
///                            loop we just completed by checking all blocks.
