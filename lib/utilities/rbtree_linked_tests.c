/**
 * File: rbtree_linked_tests.c
 * ---------------------------
 * This file contains the implementation for testing the rbtree_linked
 * allocator. We have new complexity of storing duplicate nodes in a linked list
 * attached to the tree nodes. If run in gdb these functions will stop execution
 * during the failing test in the stack frame that failed.
 */
...
/* @brief calculate_bheight  determines if every path from a node to the
 * tree.black_nil has the same number of black nodes.
 * @param *root              the root of the tree to begin searching.
 * @param *black_nil         the sentinel node at the bottom of the tree that is
 * always black.
 * @return                   -1 if the rule was not upheld, the black height if
 * the rule is held.
 */
...
/* @brief calculate_bheight_V2  verifies that the height of a red-black tree is
 * valid. This is a similar function to calculate_bheight but comes from a more
 *                              reliable source, because I saw results that made
 * me doubt V1.
 * @param *root                 the starting node of the red black tree to
 * check.
 * @param *black_nil            the sentinel node at the bottom of the tree that
 * is always black.
 * @citation                    Julienne Walker's writeup on topdown Red-Black
 * trees has a helpful function for verifying black heights.
 */
