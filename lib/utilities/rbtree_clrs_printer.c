 ...
 * ---------------------------
 * This file contains the printing utilities for the rbtree_clrs allocator.
 * Printing functions are incredibly important for the tree based
 * implementations because I use them to check the immediate effect of a
 * rotation on the tree. I also use one of these in the print_peaks program.
 */
...
/* @brief print_node  prints an individual node in its color and status as left
 * or right child.
 ...
 * @param block_size        the full size of a block of memory, not just the
 * user block size.
 */
 ...
 * @param *root           the root node of the tree to start at for an overall
 * heap check.
 * @param *black_nil      the sentinel node at the bottom of the tree that is
 * always black.
 */
