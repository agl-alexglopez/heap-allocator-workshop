 ...
 * reflect the specified status and that the left neighbor is allocated or
 * unavailable.
 ...
 * block is now free. We will only initialize footers on free blocks. We use the
 * the control bits in the right neighbor if the block is allocated and allow
 * the user to have the footer space.
...
/* @brief is_left_space  checks the control bit in the second position to see if
 * the left neighbor is allocated or free to use for coalescing.
