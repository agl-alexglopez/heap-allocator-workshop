/**
 * File list_addressorder_design.c
 * ---------------------------------
 * This file contains the declarations of utility functions for the list_addressorder heap
 * allocator. These functions serve as basic navigation for nodes and blocks based on the node
 * types and sizes we have established. External definitions need to go here to follow C standards
