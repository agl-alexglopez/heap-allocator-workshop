...
///
/// [EXE] [LINE NUMBER] [SCRIPT NAME]
///
/// Add any number of line numbers to break on.
///
/// [EXE] [LINE NUMBER] [LINE NUMBER] [LINE NUMBER] [LINE NUMBER] [SCRIPT NAME]
///
/// Add the quiet flag if you do not want the plotting pop up.
///
/// [EXE] [LINE NUMBER] [SCRIPT NAME] -q
///
...
peaks(std::span<char const *const> args) {
    try {
        ...
        for (auto const *arg : args) {
            try {
                ...
                if (std::string::npos != strarg.find(".script")) {
                    ...
                    auto err = std::string(
                                   "Valid options are: numbers for breakpoints "
                                   "or the -v for verbose flag.\nFound: ")
                                   .append(strarg)
                                   .append("\n");
        ...
        breaks.breakpoints.erase(
            std::unique(breaks.breakpoints.begin(), breaks.breakpoints.end()),
            breaks.breakpoints.end());
...
print_peaks(std::string const &script_name, user_breaks &user_reqs) {
    ...
    for (auto const &line : script.lines) {
        ...
        if (curr_breakpoint < user_reqs.breakpoints.size()
            ...
            std::cout << msg;
            handle_user_breakpoints(user_reqs,
                                    {curr_breakpoint, script.lines.size() - 1});
            ++curr_breakpoint;
        }
    ...
    // Maybe there is some clever technique to track the data structure over
    // time but I think that would expose too much internals. Running it twice
    // seems fine for now.
            ...
            if (breakpoint > lim.max || breakpoint < min) {
                auto const msg
                    = std::string("Breakpoint is past the last which is ")
                          ...
                          .append("\n");
                std::cout << msg;
            } else if (!std::binary_search(user_reqs.breakpoints.begin(),
                                           user_reqs.breakpoints.end(),
                                           breakpoint)) {
                user_reqs.breakpoints.insert(
                    std::lower_bound(user_reqs.breakpoints.begin(),
                                     user_reqs.breakpoints.end(), breakpoint),
                    breakpoint);
            }
        } catch (...) {
            if (buf.empty() || buf == "c") {
