/// Files: ctest.c
/// ---------------------
/// This is the Correctness Tester aka ctest.c. This program parses and runs
/// scripts from the provided location by the user. script::requests files are
/// helpful for performing the necessary heap operations to exercise code paths
/// but not actually introducing program logic to get in the way of checking
/// such operations. This programs provides a decent amount of correctness
/// checks based on a limited api it can gain as a user of each heap allocator.
/// However, it relies heavily on thorough implementations of the validation
/// functions written by each allocator. This framework will allow the user to
/// validate a variety of scripts with those internal debugging functions. This
/// is based upon the test_harness program written in C by jzelenski and Nick
/// Troccoli (Winter 18-19) but has been updated to C++ and modified for ease of
/// use with the rest of my testing framework. I added more information for
/// overlapping heap boundary errors and encapsulated alloc, realloc, and free
/// logic to their own functions. I also created an output progress bar system
/// so that we can see how many scripts we have left to check. Only the loading
/// bar occurs on success and we should only see output of text when something
/// fails. I have also separated script parsing out to another file because I
/// have other programs, such as the print peaks program that uses scripts.
#include "allocator.h"
#include "osync.hh"
#include "script.hh"
#include "segment.h"
...
bool eval_malloc(const script::line &line, script::requests &s,
                 void *&heap_end);
bool eval_realloc(const script::line &line, script::requests &s,
                  void *&heap_end);
...
int
test(std::span<const char *const> args)
{
    try
    {
        ...
        for (const auto *arg : args)
        {
            ...
            if (!script)
            {
                const auto err = std::string("Failed to parse script ")
                                     .append(arg)
                                     .append("\n");
                osync::syncerr(err, ansi_bred);
                return 1;
            }
            const std::optional<size_t> used_segment
                = eval_correctness(script.value());
            if (!used_segment)
            {
                const auto err
                    = std::string("Failed script ").append(arg).append("\n");
                osync::syncerr(err, ansi_bred);
                return 1;
            }
        ...
        const auto util = std::string("Utilization=")
                              .append(std::to_string(utilization / args.size()))
                              .append("%\n");
        osync::cout(util, osync::ansi_bgrn);
        return 0;
    } catch (std::exception &e)
    {
        const auto ex = std::string("Script tester exception thrown: ")
                            .append(e.what())
                            .append("\n");
...
eval_correctness(script::requests &s)
{
    ...
    for (const script::line &line : s.lines)
    {
        ...
        {
            osync::syncerr(
                "Unknown request slipped through script validation\n",
                ansi_bred);
            return {};
        }
        }
    ...
    return static_cast<uint8_t *>(heap_end_addr)
           - static_cast<uint8_t *>(heap_segment_start());
}

bool
eval_malloc(const script::line &line, script::requests &s, void *&heap_end)
{
    ...
    {
        osync::syncerr(
            "Block is overlapping another block causing heap corruption.\n",
            ansi_bred);
        return false;
    }
    const std::span<uint8_t> cur_block
        = std::span<uint8_t>{static_cast<uint8_t *>(p), line.size};
    ...
    std::fill(cur_block.begin(), cur_block.end(),
              line.block_index & lowest_byte);
    s.blocks.at(line.block_index) = {p, line.size};
    return true;
}

bool
eval_realloc(const script::line &line, script::requests &s, void *&heap_end)
{
    ...
    if (!verify_block(new_ptr, (old_size < line.size ? old_size : line.size),
                      s))
    {
        return false;
    }
...
bool
eval_free(const script::line &line, script::requests &s)
{
...
bool
verify_block(void *ptr, size_t size, const script::requests &s)
{
    ...
    auto block_end = std::span<const uint8_t>(static_cast<uint8_t *>(ptr), size)
                         .subspan(size);
    auto heap_end
        = std::span<const uint8_t>(static_cast<uint8_t *>(heap_segment_start()),
                                   heap_segment_size())
              .subspan(heap_segment_size());
    if (ptr < heap_segment_start())
    {
        std::stringstream err{};
        err << "New block ( " << ptr << ":" << block_end.data()
            << ") not within heap segment ( " << heap_segment_start() << ":"
            << heap_end.data()
            << ")\n"
               "|----block-------|\n"
               "        |------heap-------...|\n";
    ...
    for (const auto &[addr, size] : s.blocks)
    {
        ...
        auto other_end
            = std::span<const uint8_t>(static_cast<uint8_t *>(addr), size)
                  .subspan(size);
        if (ptr >= addr && ptr < other_end.data())
        {
            std::stringstream err{};
            err << "New block ( " << ptr << ":" << block_end.data()
                << ") overlaps existing block ( " << addr << ":"
                << other_end.data() << ")\n"
                << "     |------current---------|\n"
                   "|------other-------|\n"
                   "or\n"
