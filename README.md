...
I spent much of the summer of 2022 exploring the different ways that I could implement a Red-Black Tree heap allocator. I worked through three seperate implementations: the first directly from the *Introduction to Algorithms* textbook, fourth edition by Cormen, Leiserson, Rivest, and Stein (CLRS), the second a unified version that was able to unify the symmetric case for left and right to one function or case, and a third implementation that eliminates the need for the parent field by using a stack. All three of these implementations served different purposes and there were unique challenges to making a heap allocator a red-black tree. For example, how one should handle duplicate nodes or coalesce left and right if they encounter such a duplicate in either direction became interesting hurdles to overcome.
...
> **Read my code here ([`explicit_tree_clrs.c`](/assignments/assignment-06/assign6/explicit_tree_clrs.c)).**
...
> **Read my code here ([`explicit_tree_unified.c`](/assignments/assignment-06/assign6/explicit_tree_unified.c)).**
...
> **Read my code here ([`explicit_tree_unified_doubly.c`](/assignments/assignment-06/assign6/explicit_tree_unified_doubly.c)).**
...
> **Read my code here ([`explicit_tree_unified_stack.c`](/assignments/assignment-06/assign6/explicit_tree_unified_stack.c)).**
...
> **Read my code here ([`explicit_tree_unified_topdown.c`](/assignments/assignment-06/assign6/explicit_tree_unified_topdown.c)).**
...
We can overcome the insertion problem rather easily. When we encounter a duplicate, we simply perform one final fixup but instead of ending the function, we use the doubly linked list we have been employing over the last few iterations of our allocator and add the duplicate to that list. This prevents problems that would otherwise arise with the logic and sequence of fixups on the way down the tree.
